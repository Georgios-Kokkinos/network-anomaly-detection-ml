...
Entry point for Version 3 of the network anomaly detection pipeline. It orchestrates:
- Loading and cleaning UNSW-NB15 CSVs from the local data directory.
...
from src.utils.helpers import map_unsw_class_names, plot_class_distribution, plot_class_pie, save_metrics_and_report
...
NROWS = 100000  # Number of rows to read from UNSW CSV files (default is None, eg. read all rows)
...
def write_reports(metrics_df, save_dir, n_total, n_train, n_test, balanced_sizes, class_names, feature_names_pretty):
    ...
    short_report = dedent(
        ...
        Overview
        Three tree-based classifiers (RandomForest, XGBoost, LightGBM) were trained on a unified UNSW-NB15 pipeline with hybrid class balancing (downsample majority + SMOTE). A single stratified 80/20 hold-out enables fair comparison across models.
...
def main():
    ...
    class_names = map_unsw_class_names(label_encoder.inverse_transform(range(len(label_encoder.classes_))))
    ...
    for model_key, model_name in models:
        ...
        target_names = map_unsw_class_names(label_encoder.inverse_transform(range(len(label_encoder.classes_))))
