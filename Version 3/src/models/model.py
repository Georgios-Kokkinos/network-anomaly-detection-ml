...
class AnomalyDetectionModel:
    ...
    def handle_class_imbalance(self, X, y, n_samples=None, target_count=None):
        ...
        if n_samples is None:
            majority_target = len(df_majority)
        else:
            majority_target = min(int(n_samples), len(df_majority))

        df_majority_downsampled = resample(
            df_majority,
            replace=False,
            n_samples=majority_target,
            random_state=42
        )
        ...
        if sampling_strategy:
            minority_counts = class_counts[class_counts.index != class_counts.idxmax()]
            if minority_counts.empty:
                X_resampled, y_resampled = X_balanced, y_balanced
                return X_resampled, y_resampled

            min_minority = int(minority_counts.min())
            # SMOTE requires at least k_neighbors + 1 samples in every minority class.
            if min_minority < 2:
                X_resampled, y_resampled = X_balanced, y_balanced
                return X_resampled, y_resampled
